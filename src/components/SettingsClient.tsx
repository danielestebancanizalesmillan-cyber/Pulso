...
export function SettingsClient() {
    ...
    const handleDeleteAccount = async () => {
        const confirmed = window.confirm("¿Estás SEGURO de que quieres eliminar tu cuenta? Esta acción es irreversible y borrará todos tus datos.");
        if (!confirmed) return;

        const finalConfirmed = window.confirm("POR FAVOR CONFIRMA UNA VEZ MÁS. Se borrarán todos tus tweets, seguidores y perfil para siempre.");
    ...
    return (
        <div className={styles.settingsContainer}>
            ...
            {/* Danger Zone */}
            <section className={styles.settingsSection}>
                <div className={styles.sectionHeader} style={{ color: "var(--red)" }}>
                    ...
                    <h2 className={styles.settingsSectionTitle} style={{ color: "var(--red)" }}>Zona de Peligro</h2>
                </div>
                <div className={styles.settingsCard} style={{ borderColor: "var(--red-faint)" }}>
                    <div className={styles.settingsRow} onClick={handleDeleteAccount} style={{ cursor: "pointer" }}>
                        <div className={styles.rowContent}>
                            <span className={styles.rowLabel} style={{ color: "var(--red)" }}>Eliminar Cuenta</span>
                            <span className={styles.rowDesc}>Borrar permanentemente tu cuenta y todos tus datos.</span>
                        </div>
                        <svg viewBox="0 0 24 24" fill="none" stroke="var(--red)" strokeWidth="2" style={{ width: 18, height: 18 }}>
